                ...
                <div className="md:col-span-1 bg-gray-800 p-4 rounded-lg max-h-[80vh] overflow-y-auto">
                    ...
                    {pedidos.map(pedido => (
                        <div key={pedido._id} className={`p-3 rounded-lg mb-2 transition-colors ${pedidoSelecionado?._id === pedido._id ? 'bg-yellow-500/20' : 'hover:bg-gray-700'}`}>
                            <div onClick={() => setPedidoSelecionado(pedido)} className="cursor-pointer flex-1">
                            ...
                            <button onClick={(e) => { e.stopPropagation(); handleDeleteClick(pedido); }} className="mt-2 text-red-400 hover:text-red-300 text-sm flex items-center gap-1" title="Remover pedido">
                            ...
                            <motion.div key={pedidoSelecionado._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button onClick={() => handleDeleteClick(pedidoSelecionado)} className="bg-red-600 text-white font-medium py-2 px-4 rounded-lg flex items-center gap-2 hover:bg-red-700">
                                            <FaTrash className="text-sm" /> Remover
                                        </button>
                                    </div>
                                </div>
                                <div className="bg-gray-700 p-4 rounded-lg mb-4">
                                    <h4 className="text-lg font-semibold text-white mb-3 flex items-center gap-2"><FaUser className="text-yellow-500" /> Cliente</h4>
                                    <p className="text-white font-medium">{pedidoSelecionado.cliente.nome}</p>
                                    <p className="text-gray-300 text-sm">{pedidoSelecionado.cliente.telefone}</p>
                                    <p className="text-gray-300 text-sm mt-1">{formatAddress(pedidoSelecionado.endereco)}</p>
                                </div>
                                <div className="bg-gray-700 p-4 rounded-lg mb-4">
                                ...
                                <div className="mt-4 flex justify-end gap-2">
                                    {getNextStatus(pedidoSelecionado.status) && (
                                        <button onClick={() => updateOrderStatus(pedidoSelecionado._id, getNextStatus(pedidoSelecionado.status)!)} disabled={updatingStatus === pedidoSelecionado._id} className="bg-yellow-500 text-black font-bold py-2 px-4 rounded hover:bg-yellow-600 disabled:opacity-50">
                                            Avançar Status
                                        </button>
                                    )}
                                    <button onClick={() => updateOrderStatus(pedidoSelecionado._id, 'cancelado')} className="bg-red-600 text-white font-bold py-2 px-4 rounded hover:bg-red-700">
                                        Cancelar
                                    </button>
                                </div>
                            </motion.div>
            ...
            <AnimatePresence>
                {showDeleteConfirm && orderToDelete && (
                    <motion.div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
                        <motion.div className="bg-gray-800 rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
                            <h2 className="text-xl font-bold text-white mb-4">Confirmar Exclusão</h2>
                            <p className="text-gray-300 mb-6">Tem certeza que deseja remover o pedido <strong>#{orderToDelete._id.slice(-6)}</strong>?</p>
                            <div className="flex gap-3">
                                ...
                                <button onClick={() => deleteOrder(orderToDelete._id)} disabled={deletingOrder === orderToDelete._id} className="flex-1 bg-red-600 text-white font-bold py-3 px-4 rounded-lg hover:bg-red-700 disabled:opacity-50">
                                    {deletingOrder ? 'Removendo...' : 'Remover'}
                                </button>
                            </div>
                        </motion.div>
                    </motion.div>
