...
export default function MiniItemModal({ item, onClose, onAdd, categories = [] }: MiniItemModalProps) {
  ...
  useEffect(() => {
    // Bloquear scroll do body de forma simples
  ...
  const calculatePrice = () => {
    ...
    if (item.extraOptions) {
      selectedExtras.forEach(extra => {
        if (item.extraOptions && item.extraOptions[extra]) {
          price += item.extraOptions[extra];
        }
      });
    }
  ...
  // Verificar se precisa selecionar um sabor
    ...
    return (
      <AnimatePresence>
        <motion.div
          ...
          initial={isLowEndDevice ? {} : { opacity: 0 }}
          animate={isLowEndDevice ? {} : { opacity: 1 }}
          exit={isLowEndDevice ? {} : { opacity: 0 }}
          transition={isLowEndDevice ? undefined : { duration: 0.2 }}
          onClick={onClose}
        >
          <motion.div
            data-modal-content
            className="bg-gray-900 rounded-2xl w-full max-w-md mx-2 shadow-2xl border border-red-500/30 overflow-hidden max-h-[90vh] overflow-y-auto"
            initial={isLowEndDevice ? {} : { scale: 0.95, opacity: 0 }}
            animate={isLowEndDevice ? {} : { scale: 1, opacity: 1 }}
            exit={isLowEndDevice ? {} : { scale: 0.95, opacity: 0 }}
            transition={isLowEndDevice ? undefined : { duration: 0.2 }}
          ...
          >
            <div className="relative h-32 sm:h-48 bg-gradient-to-br from-red-500/20 to-red-600/20">
              ...
              <button
                onClick={onClose}
                className="absolute top-3 right-3 w-8 h-8 bg-gray-900/80 rounded-full flex items-center justify-center text-white hover:bg-gray-800 transition-colors"
              >
                <FaTimes />
              </button>
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:gap-3">
                    {item.flavors.map((flavor) => (
                      <button
                        ...
                        className={`p-2 sm:p-3 rounded-lg border-2 transition-colors text-xs sm:text-sm text-left ${!flavor.available
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-3">
                      {Object.entries(item.extraOptions).map(([extra, price]) => (
                        <button
                          ...
                          className={`p-2 sm:p-3 rounded-lg border-2 transition-colors text-xs sm:text-sm ${selectedExtras.includes(extra)
                            ? 'border-yellow-500 bg-yellow-500/20 text-yellow-400'
                            : 'border-gray-700 hover:border-yellow-500 text-gray-300'
                            }`}
                        >
              ...
              <div>
                ...
                <div className="flex items-center justify-center gap-3 sm:gap-4">
                  <button
                    ...
                    disabled={quantity <= 1}
                    className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center transition-colors ${quantity <= 1
                        ? 'bg-gray-800 text-gray-500 border-2 border-gray-700 cursor-not-allowed'
                        : 'bg-gray-800 text-yellow-500 hover:bg-gray-700 border-2 border-gray-700 hover:border-yellow-500'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gray-800 text-yellow-500 hover:bg-gray-700 border-2 border-gray-700 hover:border-yellow-500 transition-colors flex items-center justify-center"
                  >
                ...
                <div className="flex gap-2 sm:gap-3">
                  ...
                  {item.available && (
                    <button
                      ...
                      className={`flex-1 py-2.5 sm:py-3 px-3 sm:px-4 rounded-lg transition-colors font-bold shadow-lg text-sm sm:text-base ${canAddToCart
